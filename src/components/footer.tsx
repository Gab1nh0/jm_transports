...
function AccordionCol({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  return (
    <div className={`footer-col footer-col--accordion ${open ? 'open' : ''}`}>
      <button className="footer-col__toggle" onClick={() => setOpen(o => !o)}>
        <h4>{title}</h4>
        <svg className="footer-col__chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <polyline points="6 9 12 15 18 9" />
        </svg>
      </button>
      <div className="footer-col__body">{children}</div>
    </div>
  );
}
  ...
  return (
    <footer className="footer-root">
      <div className="footer-main">
        ...
        {/* Quick Links */}
        <AccordionCol title={t('footer.quicklinks')}>
          <ul>
        ...
        {/* Services */}
        <AccordionCol title={t('footer.services')}>
          <ul>
        ...
        {/* Location */}
        <AccordionCol title={t('footer.location')}>
